...
document.addEventListener("DOMContentLoaded", () => {
    ...
    addGlobalEventListener(
        ...
        async (e) => {          // Callback function to handle the event
            ...
            /*
            Find and display rhymes from the local JSON
            ...
            try {
                ...
                if (rhymesMatchFromJson.length === 0) {
                    // Log that no rhymes were found in JSON
                    console.log(`No rhymes found in JSON for "${wordToRhyme}".`);

                    // Add a fallback message to the rhymes result array
                    rhymesResultArr.push('No rhymes available for the provided word.');
                } else {
